...
2021년 12월, 나는 카사 백엔드 엔지니어로 입사했다. 그 때, 회사가 막 싱가포르로 사업을 확장하기 시작한 시점이었다. 나는 이 회사에 입사한 이유가 2가지였다.

첫째는 `글로벌 사업`을 경험할 수 있다는 것이었다. 항상 글로벌 사업에 관심이 있었고, 전직장에서는 글로벌 사업에 참여하지 못해서 이직 결정을 내리기도 했다.

둘째는 `부동산 조각 투자`라는 `프롭테크 도메인`을 경험해 볼 수 있다는 것이었다. 일반적인 부동산 투자가 아닌, 부동산을 조각내어 투자하는 사업이 매우 흥미로웠다. 우리 팀의 역할은 싱가포르 버전의 제품을 개발하는 것이다.
...
2023년 1월, 카사의 매각 소식을 접하게 되었다. 부분 매각으로 코리아 프로덕트 팀이 대상이 되었으며, 싱가포르 프로덕트 팀은 남게 되었다. 이전에는 60명이 근무했던 회사가 이번 매각으로 10명으로 축소되었으며, 이로 인해 남은 인원들은 기존 인원들이 맡았던 업무를 대신 맡게 되었다. 그 중에서도 제가 맡게 된 업무는 인프라 업무를 서포트하는 `DevOps` 업무다.

새로운 일을 맡게 된 것에는 염려도 되는 부분이 있지만, 이전의 경험을 회고해보는 것이 중요하다고 느꼈다. 따라서 길고 상세한 소개글이었지만, 이러한 이유로 회고를 작성하게 되었다.
...
FE가 API를 호출할 때 `JWT`를 사용하여 Token `Authentication`을 개발하여, `access token`으로 요청하고 만료되면 `refresh token`을 사용하여 새로운 access token을 발급하는 구조를 선택했다. 이를 위해 Django rest framework의 `simplejwt` 라이브러리를 사용하여 토큰 생성, revoke 및 refresh를 구현했다.

회사에서는 3단계의 로그인 절차(ID/PW 로그인, OTP 로그인, 투자 계좌 로그인)가 필요하다. 즉, 투자 계좌 로그인까지 완료해야만 서비스를 정식으로 사용할 수 있다. 이를 위해 각 단계에서 토큰을 발급한다.
...
처음에는 토큰의 유효 기간이나 폐기등의 토큰 정책을 유념하지 않았다. 하지만 `싱가포르 금융감독기관`의 인증인 `MAS Regulation`을 준수하기 위해랙 서버 보안 강화해야 했고, 토큰 정책도 포함이 되어있었다.
...
refresh 토큰 수명이 짧아지면, 사용자가 활동이 없을 때 쉽게 로그아웃 되는 단점이 있지만, 우리는 앱이 아닌 웹 서비스이기 때문에 싱가포르 금융 규제상 비교적 짧은 토큰 수명을 가지게 되었다. 
