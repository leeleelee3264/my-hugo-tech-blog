...
프러덕트 팀에서는 법적인 사항으로 `런칭에 병목이 걸릴 거라 예상`했지만 1시간도 안걸려 수정하고 배포까지 끝냈기 떄문에 런칭 일정에 차질이 생기지 않았다. 이 경험으로 초기에 구현을 할 떄 왜 많은 개발자들이 유지보수에 용이하도록 읽기 쉬운 코드를 짜고, 추상화를 하라고 했는지 몸소 이해할 수 있었다. 
